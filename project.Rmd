...
The training and test data are obtained from <ttps://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv> and <https://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv> respectively.
...
summary(pca)
```
We see that with only 14 principal components we almost encapture 100% of the information.
Let's run the preProcess function in the caret package and set the number of pc to use to 14 (pcaComp=14); and get the new rotated training and test set.
```{r}
...
```
The Out of sample error for this model is as stated in modFit$finalModel 3.68%
```{r}
modFit$finalModel
```
